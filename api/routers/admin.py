...
Provides endpoints for user management including role updates,
ban/unban operations, and viewing user activity.
"""
...
async def get_all_users(x_user_email: Optional[str] = Header(None)):
    ...
    users = await db.user.find_many(order={"role": "desc"})

    # Manually count relations for each user
    result = []
    for user in users:
        # Count user asceticisms
        asceticisms_count = await db.userasceticism.count(where={"userId": user.id})

        # Count group memberships
        groups_count = await db.groupmember.count(where={"userId": user.id})
...
async def update_user_role(
    request: UpdateRoleRequest, x_user_email: Optional[str] = Header(None)
):
    """
    ...
    await db.user.update(where={"id": request.userId}, data={"role": new_role})
...
async def toggle_user_ban(
    request: ToggleBanRequest, x_user_email: Optional[str] = Header(None)
):
    """
    Ban or unban a user.
    ...
    await db.user.update(
        where={"id": request.userId}, data={"isBanned": request.isBanned}
    )
